...
export const useRunStore = defineStore('run', () => {
  ...
  async function run() {
    ...
    try {
      const result = await executor.value.run(job, bytes, {
        ...
        onProgress: (_i, ratio) => { segmentRatio.value = ratio },
  ...
  return {
    status, segmentIndex, segmentTotal, segmentRatio, logs, outputs, error,
    backendId, executor, running, run, cancel, reset,
  }
})
