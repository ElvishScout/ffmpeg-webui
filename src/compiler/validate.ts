...
export function outputPads(node: WorkflowNode): PortSpec[] {
  switch (node.kind) {
    ...
    case 'filter': {
      ...
      if (!spec) return []
      if (spec.outputsFrom) {
        const n = node.outputCount ?? Number(node.params?.[spec.outputsFrom]) ?? spec.outputs.length
...
export function edgePortTypes(
  ...
  return { from: outputPads(src)[fromPad]?.type, to: inputPads(dst)[toPad]?.type }
}
...
export function validateGraph(
  ...
  for (const e of graph.edges) {
    ...
    if (from && to && !portsCompatible(from, to)) {
      errors.push({ code: 'portType', nodeId: e.target, nodeName: nodeDisplayName(byId.get(e.target)!) })
    }
  }
    ...
    switch (node.kind) {
      case 'asset':
        if (node.assetRef && ctx.missingAssetIds?.has(node.assetRef.id)) {
          errors.push({ code: 'assetMissing', nodeId: node.id, nodeName: name })
        }
        break
      case 'filter': {
        if (!node.filterName || !filterByName.has(node.filterName)) {
          errors.push({ code: 'filterUnknown', nodeId: node.id, nodeName: name })
          break
        }
        const pads = inputPads(node)
        for (let i = 0; i < pads.length; i++) {
          if (!connectedIn.has(`in-${i}`)) {
            errors.push({ code: 'inputUnconnected', nodeId: node.id, nodeName: name })
            break
          }
        }
        ...
        for (let i = 0; i < outs.length; i++) {
          if (!connectedOut.has(`out-${i}`)) {
            errors.push({ code: 'outputUnconnected', nodeId: node.id, nodeName: name })
            break
          }
        }
        break
