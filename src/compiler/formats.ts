...
const VP8: Partial<Record<EncodePreset, string[]>> = {
  ...
  fast: ['-c:v', 'libvpx', '-crf', '14', '-b:v', '700k', '-deadline', 'realtime', '-cpu-used', '5', '-pix_fmt', 'yuv420p'],
}
...
export const FORMATS: Record<OutputFormat, FormatSpec> = {
  ...
  webm: {
    ...
    video: VP8,
    audio: { high: ['-c:a', 'libopus', '-b:a', '128k'], fast: ['-c:a', 'libopus', '-b:a', '96k'] },
  },
  hevc: {
    kind: 'av',
    ext: 'mp4',
    presets: ['copy'],
    fixedVideo: ['-c:v', 'libx265', '-crf', '23', '-preset', 'medium', '-pix_fmt', 'yuv420p', '-tag:v', 'hvc1'],
  ...
  mp3: { kind: 'audio', ext: 'mp3', presets: ['copy'], fixedAudio: ['-c:a', 'libmp3lame', '-q:a', '2'] },
  m4a: { kind: 'audio', ext: 'm4a', presets: ['copy'], fixedAudio: ['-c:a', 'aac', '-b:a', '192k'] },
  flac: { kind: 'audio', ext: 'flac', presets: ['copy'], fixedAudio: ['-c:a', 'flac'] },
  wav: { kind: 'audio', ext: 'wav', presets: ['copy'], fixedAudio: ['-c:a', 'pcm_s16le'] },
  opus: { kind: 'audio', ext: 'opus', presets: ['copy'], fixedAudio: ['-c:a', 'libopus', '-b:a', '128k'] },
  // gif gets a palettegen/paletteuse chain injected by compile.ts, no codec args
  gif: { kind: 'video', ext: 'gif', presets: [] },
  apng: { kind: 'video', ext: 'apng', presets: [], fixedVideo: ['-c:v', 'apng'] },
}
