...
function onDragStart(e: DragEvent, a: { id: string; filename: string; size: number; mime: string }) {
  e.dataTransfer?.setData(
    'application/ffmpeg-webui',
    JSON.stringify({ type: 'asset', id: a.id, filename: a.filename, size: a.size, mime: a.mime }),
  )
}
...
const kindLabel = (kind: string) =>
  t(`assets.type${kind[0].toUpperCase()}${kind.slice(1)}`)
</script>

<template>
  <div class="assets" @drop.prevent="onDrop" @dragover.prevent>
    <label class="assets__upload">
      <NButton size="small" type="primary" dashed block tag="span">{{ t('assets.upload') }}</NButton>
      <input type="file" hidden multiple @change="onPick" />
    </label>

    <NEmpty v-if="!store.assets.length" size="small" :description="t('assets.empty')" class="assets__empty" />
      ...
      >
        ...
        <div class="assets__actions">
          <NButton
            ...
            @click="graphStore.addAssetNode({ id: a.id, filename: a.filename, size: a.size, mime: a.mime })"
          >＋</NButton>
    ...
    <div class="assets__quota">
      <span v-if="store.quota.quota">
        {{ t('assets.quota', { used: fmtSize(store.quota.used), quota: fmtSize(store.quota.quota) }) }}
      </span>
...
.assets__upload { display: block; margin: 0 10px 4px; }
.assets__upload :deep(.n-button) { width: 100%; }
.assets__empty { margin: 32px 10px 0; }
.assets__list {
  flex: 1;
  min-height: 0;
  overflow-y: auto;
...
.assets__item:hover { border-color: #3f3f4d; }
.assets__info { min-width: 0; flex: 1; }
.assets__name {
  font-size: 12px;
  overflow: hidden;
...
.assets__actions { display: flex; gap: 2px; flex: none; }
.assets__quota { font-size: 11px; color: #6b7280; margin: 0 10px; }
</style>
